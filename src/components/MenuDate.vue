...
<router-link :to="'/date/' + _id" class="text-dark">
  <div class="my-3">
    <div class="food-image" :style="{backgroundImage: (dish && dish.image) ? ('url(' + dish.image + ')') : null}">
...
export default {
  computed: {
    ...
    dish() {
      if (this.date && this.date.dishIds && this.date.dishIds.length) return this.$store.state.dishes[this.date.dishIds[0]]
      else return null
    }
  },
  props: ["_id", "title"]
